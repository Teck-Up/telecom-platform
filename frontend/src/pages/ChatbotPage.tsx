...
// Aligné avec le backend FastAPI (prefix /api)
...
export default function ChatbotPage() {
  ...
  const send = async () => {
    ...
    try {
      // Correction de l'endpoint : /ia/chat -> /ask
      // Correction du payload : message -> text
      const { data } = await axios.post(`${AI_URL}/ask`, {
        text: userMsg
      })

      // Correction de la réponse : data.response -> data.answer
      setMessages(prev => [...prev, { role: 'assistant', content: data.answer }])
    } catch (error) {
      console.error('Chatbot error:', error)
      setMessages(prev => [...prev, { role: 'assistant', content: 'Désolé, je suis temporairement indisponible. Veuillez vous assurer qu\'un document PDF a été chargé.' }])
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="flex flex-col h-full p-6">
        ...
        <div className="flex-1 bg-white rounded-xl shadow-sm flex flex-col overflow-hidden">
          ...
          <div className="p-4 border-t">
            ...
            <p className="text-xs text-gray-400 mt-2">Questions suggérées: "Quelles sont les obligations ?" · "Comment contacter le support ?"</p>
          </div>
        </div>
      </div>
  )
